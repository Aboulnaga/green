...
export default function old_SignupPage() {
  ...
  const createUser = async (data: any) => {
    try {
      ...
      if (newUser.user.uid) {
        ...
        const userData = {
          user_name: newUser.user.displayName,
          user_id: newUser.user.uid,
          user_email: newUser.user.email,
          is_verfied: newUser.user.emailVerified,
          user_createdAT: Timestamp.fromDate(new Date("December 10, 1815")),
          user_updatedAT: Timestamp.fromDate(new Date("December 10, 1815")),
          user_avatar: newUser.user.photoURL,
          user_role: "customer",
        };

        // console.log(userData);

        const addNewUserToDbRes = await setDoc(
          doc(db, "users", newUser.user.uid),
          userData
        );

        console.log(addNewUserToDbRes);
      } else {
        setIsError([
          {
            error: "Something went wrong, can't create user, try again",
            path: "server",
          },
        ]);
      }
